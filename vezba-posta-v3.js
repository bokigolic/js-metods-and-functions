/*
ZADATAK na temu poste dostave paketa... VERZIJA 3
...
for (let i = 0; i < paketi.length; i++) {
  ...
  const gradPrimaoc = paket;
  // if (paket === "novi sad" || paket === "subotica") {
  if (gradoviOdRegionaSever.includes(gradPrimaoc)) {
    paketiZaSever.push(gradPrimaoc)
  } else if (gradoviOdRegionaCentar.includes(gradPrimaoc)) {
    paketiZaCentar.push(gradPrimaoc)
  } else if (gradoviOdRegionaJug.includes(gradPrimaoc)) {
    paketiZaJug.push(gradPrimaoc)
  } else {
    paketiProblematicni.push(gradPrimaoc)
  }
}
